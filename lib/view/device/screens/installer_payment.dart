...
class InstallerPaymentScreen extends StatefulWidget {
  const InstallerPaymentScreen({super.key});

  @override
  InstallerPaymentScreenState createState() => InstallerPaymentScreenState();
}

class InstallerPaymentScreenState extends State<InstallerPaymentScreen> {
  ...
  Future<void> onAprroveSubscription(dynamic data) async {
    try {
      await paymentApi.paymentComplete({
        "user_id": dataController.installerPaymentUserId.value,
        'itemId': dataController.currentPaymentItemId.value,
  ...
  Future<void> createSubscriptionFlow() async {
    if (authController.installerKey == null) return;

    try {
      String amount = dataController.onetimePaymentAmount.value;
      // Step 4: Load the WebView with the dynamically created HTML
      String dynamicHtml =
          "${ApiConfig.paypalOneTimePaymentUrl}?amount=$amount";
  ...
  Widget _orderDetails() {
    return Column(
      children: [
        _detailRow('Order ID:', orderDetails['orderID']),
        const SizedBox(height: 10),
        _detailRow(
            'Order Date:', convertUTCtoLocal(orderDetails['create_time'])),
      ],
    );
  }
