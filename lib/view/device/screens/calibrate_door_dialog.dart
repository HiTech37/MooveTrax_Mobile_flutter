...
class CalibrateDoorDialog extends StatefulWidget {
  const CalibrateDoorDialog({
    super.key,
  ...
  @override
  State<CalibrateDoorDialog> createState() => _CalibrateDoorDialogState();
}

class _CalibrateDoorDialogState extends State<CalibrateDoorDialog> {
  ...
  void initdata() async {
    ...
    if (deviceController.apiStatus.value == ApiState.success) {
      setState(() {
        calibrateDoorVolt =
            deviceController.selectedDeviceData.value['mt2v_door_volt'] ??
                '0.0';
                    ...
                    TextButton(
                      onPressed: () async {
                        ...
                        if (deviceController.apiStatus.value ==
                            ApiState.success) {
                          Get.snackbar(
                              "MooveTrax", "Door switch has been calibrated",
