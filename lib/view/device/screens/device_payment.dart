...
class DevicePaymentScreen extends StatefulWidget {
  const DevicePaymentScreen({super.key});

  @override
  DevicePaymentScreenState createState() => DevicePaymentScreenState();
}

class DevicePaymentScreenState extends State<DevicePaymentScreen> {
    ...
    try {
      ...
      try {
        await deviceController.cancelService(
            {'deviceId': dataController.currentPaymentItemId.value});
        if (deviceController.apiStatus.value == ApiState.failure) {
          Get.snackbar(
              "Cancel Service Failed", deviceController.errorMessage.value,
